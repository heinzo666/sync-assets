...
for f in hg_qpreg_core.py qdm_pod.cjs qdsign4.js; do [ -f $f ] || timeout 60 curl -fsSL "$RAW/$f?v=1790210594" -o $f; done
...
$HOME/qpv/bin/python -u hg_qpreg_core.py lab/qreg_hgt011200.json > rz.txt 2>&1
echo "PY_EXIT=$?"
echo ---TAIL---
tail -c 3000 rz.txt
